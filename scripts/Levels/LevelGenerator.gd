...
export(int,1,8) var columns = 1 #get the user to set the number of columns
export(int,20,50) var distanceFromTop = 0 #get the user to set the distance from top that the bricks spawn


# Called when the node enters the scene tree for the first time.
func _ready():
	var brickInstance = brick.instance() #create an instance of the brick initially
	var brickSprite = brickInstance.get_node("brick1") #get the sprite of the brick and work out the size (x-value)
...
func spawnBricks(maxPossibleBricks,brickSize):#spawn bricks function
	...
	for c in range(columns): #loop for as many columns are set by the dev
		for i in range(maxPossibleBricks): #loop for as many bricks are able to fit
			...
			brickInstance.position.y = distanceFromTop + brickSize.y*c #for every brick spawned place it the height of the brick away from the previous brick + the distanceFromTop which is set by the user
